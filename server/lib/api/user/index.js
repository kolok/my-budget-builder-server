...
.post('/', async ctx => {
  try {
    let user = await User.create(ctx.request.body); // Sign token
...
.post('/company', async ctx => {
  try {
    ...
    let company = await Company.create({
      name: ctx.request.body.companyname
    });
    var userFields = ctx.request.body;
    userFields.companyID = company._id;
    let user = await User.create(userFields); // Sign token
...
.get('/:id', auth.isAuthenticated, async ctx => {
  try {
    let user = await User.findById({
      _id: ctx.params.id
    }, '-password'); // Handle not found error
